...
func (c *Client) readPump() {
	...
	for {
		...
		case websocket.MessageText:
			...
			if isServerOnlyMessage(msg.Type) {
				// Presence, lock state, acknowledgements, and errors are generated by the server.
				continue
			}
...
func canSubmitOperation(canEdit bool, messageType string) bool {
	switch messageType {
	case MsgTypeSync, MsgTypeLock, MsgTypeUnlock:
		return canEdit
	default:
		return true
	}
...
func isServerOnlyMessage(messageType string) bool {
	switch messageType {
	case MsgTypePresence, MsgTypeLockState, MsgTypeAck, MsgTypeError:
		return true
	default:
		return false
	}
