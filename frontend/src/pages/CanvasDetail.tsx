          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-6">
              ...
              <div className="mt-4 flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <Button variant="ghost" size="sm" loading={forking} onClick={() => void handleFork()}><GitFork size={16} /> {t('Fork')}</Button>
              </div>

              <div className="mt-5">
                <h3 className="mb-3 font-semibold text-slate-900">{t('Comments ({count})', { count: comments.length })}</h3>
                {user ? (
                  ...
                  <div className="space-y-4">
                    {comments.map(comment => (
                      <div key={comment.id} className="flex gap-3">
                        <Avatar name={comment.author_name || t('User #{id}', { id: comment.author_id })} size="sm" />
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="text-sm font-medium text-slate-900">{comment.author_name || t('User #{id}', { id: comment.author_id })}</span>
                            <span className="text-xs text-slate-400">{new Date(comment.created_at * 1000).toLocaleDateString(language === 'zh' ? 'zh-CN' : 'en-US')}</span>
                          </div>
