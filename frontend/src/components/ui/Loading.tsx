  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="flex flex-col items-center">
        ...
        <p className="mt-1 text-xs text-slate-400">Preparing your collaborative canvas</p>
      </div>
    </div>
  );
}
...
export function CanvasGridSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Loading canvases">
...
export function ErrorState({ title, message, onRetry }: { title: string; message: string; onRetry?: () => void }) {
  return (
    <div className="rounded-2xl border border-red-100 bg-red-50/70 px-6 py-10 text-center">
      ...
      {onRetry && <Button variant="secondary" className="mt-4" onClick={onRetry}><RefreshCw size={15} />Try again</Button>}
    </div>
  );
}
